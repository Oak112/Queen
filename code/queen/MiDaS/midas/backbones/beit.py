...
def block_forward(self, x, resolution, shared_rel_pos_bias: Optional[torch.Tensor] = None):
	    """Compatibility wrapper around timm's ``Block.forward``.

	    Upstream MiDaS assumes that the timm Block exposes a ``drop_path``
	    attribute. Some timm versions (or alternative Block implementations)
	    might not define ``drop_path``; in that case we simply bypass it and
	    apply the attention/MLP outputs directly. This keeps the behaviour
	    close to the intended one while avoiding attribute errors.
	    """
	    # ``drop_path`` may not exist depending on the timm version.
	    drop_path = getattr(self, "drop_path", None)
	
	    if self.gamma_1 is None:
	        attn_out = self.attn(self.norm1(x), resolution,
	                             shared_rel_pos_bias=shared_rel_pos_bias)
	        mlp_out = self.mlp(self.norm2(x))
	        if drop_path is not None:
	            x = x + drop_path(attn_out)
	            x = x + drop_path(mlp_out)
	        else:
	            x = x + attn_out
	            x = x + mlp_out
	    else:
	        attn_out = self.gamma_1 * self.attn(
