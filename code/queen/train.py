            ...
            with torch.no_grad():
                ...
                with torch.no_grad():
                    ...
                    if (opt.iterations - iteration)<(n_cams) and cam_idx == 0 and (dataset.log_images or dataset.log_compressed or dataset.log_ply):
                        ...
                        # For COLMAP-based scenes, ``video_cameras`` is built from
                        # the (static) train camera set and can be shorter than the
                        # temporal sequence length ``n_frames`` used for training.
                        # The original QUEEN code indexed this list with
                        # ``frame_idx-1``, which can raise ``IndexError`` when
                        # ``n_frames > len(video_cameras)`` (as in our dataset2
                        # setup where COLMAP is only available for a single
                        # reference frame).  To keep behaviour identical for the
                        # standard DyNeRF/Immersive pipelines (where
                        # ``len(video_cameras) == n_frames``) but robust for
                        # COLMAP scenes, we clamp the index to the valid range.
                        # Spiral logging is purely visualization. On some clusters/GPUs,
                        # a transient CUDA OOM can happen here (e.g., due to extension
                        # mismatch or memory fragmentation). We guard this path so
                        # training does not abort when the spiral render fails.
                        if len(video_cameras) > 0:
                            try:
                                video_idx = min(frame_idx - 1, len(video_cameras) - 1)
                                video_camera = video_cameras[video_idx]
                                ...
                                save_image(
                                    torch.clip(spiral_img, 0.0, 1.0),
                                    os.path.join(
                                        dataset.model_path,
                                        "spiral",
                                        f"{str(dataset.start_idx + frame_idx).zfill(4)}.png",
                                    ),
                                )
                            except RuntimeError as e:
                                if "out of memory" in str(e).lower():
                                    print(
                                        f"[WARN] Spiral render OOM at frame {frame_idx}, "
                                        f"skipping spiral image and continuing. ({e})"
                                    )
